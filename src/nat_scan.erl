...
start() ->
    ...
    {ok, HttpcProfile} = inets:start(httpc, [{profile, nat}], stand_alone),
    _ = natupnp_v1(HttpcProfile),
    _ = natupnp_v2(HttpcProfile),
    _ = natpmp(HttpcProfile),
...
natupnp_v1(HttpcProfile) ->
    ?LOG("[natupnp_v1] discovering", []),
    case natupnp_v1:discover() of
        {ok, Context} ->
            ?LOG("[natupnp_v1] discovered ~p", [Context]),

            case natupnp_v1:add_port_mapping(Context, tcp, 8333, 8333, 3600, HttpcProfile) of
...
natupnp_v2(HttpcProfile) ->
    ?LOG("[natupnp_v2] discovering", []),
    case natupnp_v2:discover() of
        {ok, Context} ->
            ?LOG("[natupnp_v2] discovered ~p", [Context]),

            case natupnp_v2:add_port_mapping(Context, tcp, 8333, 8333, 3600, HttpcProfile) of
                {ok, _Since, _InternalPort, _ExternalPort, _MappingLifetime}=OK ->
...
natpmp(HttpcProfile) ->
    ...
    case natpmp:discover() of
        {ok, Context} ->
            ...
            case natpmp:add_port_mapping(Context, tcp, 8333, 8333, 3600, HttpcProfile) of
                {ok, _Since, _InternalPort, _ExternalPort, _MappingLifetime}=OK ->
